...
AnimatorTask.prototype.Start=function(){if(this.OnTaskStart)this.OnTaskStart(this);if(this.Condition&&!this.Condition(this)){this.DoStop(false);return;}
if(this.Delay>0&&!this.IsSkipToEnd()){this.SetStartAndEndValue();if(this.Speed!=0){this.TimeSpan=Math.abs((this.EndValue-this.StartValue)/this.Speed*1000);}
if(this.CheckEnd(-1,false))return;var me=this;this.DelayTimerId=this.Root.ReqTimer(function(){me.DoStart();},this.Delay*Animations.TimeStrech);}else{this.DoStart();}}
AnimatorTask.prototype.SetStartAndEndValue=function(){if(this.StartValueFromModel){this.Value=this.StartValue=this.GetValue();}else{this.Value=this.StartValue;}
if(this.EndValueFunc)this.EndValue=this.EndValueFunc(this);}
AnimatorTask.prototype.DoStart=function(){if(this.DelayTimerId>=0){this.Root.CancelTimer(this.DelayTimerId);this.DelayTimerId=-1;}
...
AnimatorTask.prototype.Stop=function(){if(this.StopValue){this.EndValue=this.StopValue;this.DoStop(true,true);}else{this.DoStop(false,true);}
if(this.OnStop)this.OnStop(task);}
AnimatorTask.prototype.DoStop=function(setToEnd,skipCallback){skipCallback=xDefBool(skipCallback,false);this.StartTime=0;if(this.DelayTimerId>=0){this.Root.CancelTimer(this.DelayTimerId);this.DelayTimerId=-1;}
...
AnimatorTask.prototype.Next=function(){this.FrameId=-1;var sweepPos=1;if(this.ValueType=='number'||this.ValueType=='int'){var time=xTimeMS()-this.StartTime;if(this.TimeSpan!=0&&!this.IsSkipToEnd()){sweepPos=time/(this.TimeSpan*Animations.TimeStrech);if(sweepPos>1)sweepPos=1;var sweepValue=sweepPos;if(this.SweepFunc){sweepValue=this.SweepFunc(sweepPos);}
this.Value=sweepValue*(this.EndValue-this.StartValue)+this.StartValue;}else{this.Value=this.EndValue;}
if(this.ValueType=='int'){var offset=this.EndValue>this.StartValue?-0.4999:0.4999;this.Value=Math.round(this.Value+offset);}}else if(this.ValueType=='bool'){this.Value=this.EndValue;}else if(this.ValueType=='string'){this.Value=this.EndValue;}
...
AnimatorTask.prototype.CheckEnd=function(sweepPos,checkTimes){var isEnd;if(this.ValueType=='number'||this.ValueType=='int'){if(sweepPos>=0){isEnd=sweepPos>=1;}else{if(checkTimes&&this.TimeSpan==0&&this.Speed==0){isEnd=true;}else if(xNum(this.StartValue)&&xNum(this.Value)){isEnd=this.EndValue>=this.StartValue?this.Value>=this.EndValue:this.Value<=this.EndValue;}else{isEnd=false;}}}else if(this.ValueType=='bool'){if(checkTimes&&this.TimeSpan==0&&this.Speed==0){isEnd=true;}else{isEnd=(xBool(this.Value)&&this.Value==this.EndValue);}}else if(this.ValueType=='string'){if(checkTimes&&this.TimeSpan==0&&this.Speed==0){isEnd=true;}else{isEnd=(xStr(this.Value)&&this.Value==this.EndValue);}}
if(isEnd)this.DoStop(true);return isEnd;}
function Animator(params,parent){params=xDefObj(params,{});parent=xDefObj(parent,null);this.Parent=parent;this.Root=parent?parent.GetRoot():null;this.Id=-1;this.Mode=xDefStr(params.Mode,'serial');this.ModelRef=xDefAny(params.ModelRef,null);this.Delay=xDefNum(params.Delay,0);this.OnModelChange=xDefFunc(params.OnModelChange,null);this.RestartAction=xDefStr(params.RestartAction,'stop');this.AutoInit=xDefBool(params.AutoInit,true);this.OnTaskStart=xDefFunc(params.OnTaskStart,null);this.OnTaskEnd=xDefFunc(params.OnTaskEnd,null);this.OnStop=xDefFunc(params.OnStop,null);this.Condition=xDefFunc(params.Condition,null);this.SetValueFunc=xDefFunc(params.SetValueFunc,null);this.GetValueFunc=xDefFunc(params.GetValueFunc,null);this.Repeat=xDefNum(params.Repeat,0);this.LoopIterator=xDefAny(params.LoopIterator,null);this.OnLoopInit=xDefFunc(params.OnLoopInit,null);this.OnLoopBegin=xDefFunc(params.OnLoopBegin,null);this.OnLoopEnd=xDefFunc(params.OnLoopEnd,null);this.TaskList=[];this.TaskIsActiveList=[];this.NumTasks=0;this.CurrTask=0;this.TimerList=[];this.TimerCallbackList=[];this.FinishedCount=0;this.ModelRefObj=null;this.ModelRefList=[];this.ValueRefList=[];this.UnitList=[];this.NumChangedValues=0;this.FrameCallbackList=[];this.AnimationFrame=null;this.RestartTimerId=null;this.DelayTimerId=-1;this.IsInitialized=false;this.StartIsActive=false;this.IsRunning=false;this.IsEnding=false;this.SkipToEnd=false;if(this.AutoInit&&this.IsRoot()){var me=this;xOnDomReady(function(){me.Init();});}
if(params.AutoStart&&this.IsRoot()){var me=this;xOnLoad(function(){me.Start();});}}
Animator.prototype.AddTask=function(task){task.Id=this.NumTasks;this.NumTasks++;this.TaskList.push(task);this.TaskIsActiveList.push(false);if(this.IsInitialized&&this.AutoInit&&this.IsRoot()){task.Init();}
return this;}
Animator.prototype.Init=function(){if(this.IsInitialized)return
...
Animator.prototype.Free=function(){if(this.IsInitialized){for(var i=0;i<this.NumTasks;i++){this.TaskList[i].Free();}
...
Animator.prototype.FindValueRef=function(modelRef,valueRef,unit){if(!xDef(unit))unit=null;var numRefs=this.ModelRefList.length;for(var id=0;id<numRefs;id++){if(this.ModelRefList[id]==modelRef&&this.ValueRefList[id]==valueRef&&this.UnitList[id]==unit)return id;}
return-1;}
Animator.prototype.GetObjRef=function(base,ref){var objRef=base;if(xStr(ref)){ref=ref.replace(/\[([^\]]+)\]/g,'.$1');var refList=ref.split('.');var start=0;if(refList[0]=='DOM'){objRef=xGet(refList[1]);start+=2;}else if(refList[0]==''){start++;}
for(var i=start;i<refList.length;i++){objRef=objRef[refList[i]];}}else if(xObj(ref)){objRef=ref;}
return objRef;}
Animator.prototype.ReqValueRef=function(modelRef,valueRef,unit){var objRef=this.GetObjRef(this.ModelRefObj,modelRef);valueRef=valueRef.replace(/\[([^\]]+)\]/g,'.$1');var refParts=valueRef.split('.');var last=refParts.length-1;for(var i=0;i<last;i++){objRef=objRef[refParts[i]];}
...
Animator.prototype.GetValue=function(valueRefId){var modelRef=this.GetModel(valueRefId);var value=modelRef[this.ValueRefList[valueRefId]];if(xStr(value)&&value!=''&&this.UnitList[valueRefId]){value=parseFloat(value);}
return value;}
...
Animator.prototype.Start=function(){if(this.IsRoot()){if(!this.IsInitialized)this.Init();if(this.StartIsActive){if(!this.RestartTimerId){var me=this;this.RestartTimerId=requestAnimationFrame(function(){me.Start();});}
return;}
...
Animator.prototype.DoStart=function(){this.IsEnding=false;if(this.Mode=='parallel'){this.FinishedCount=0;for(var i=0;i<this.NumTasks;i++){this.TaskIsActiveList[i]=true;this.TaskList[i].Start();}}else{this.CurrTask=0;if(this.CurrTask<this.NumTasks){this.TaskIsActiveList[this.CurrTask]=true;this.TaskList[this.CurrTask].Start();}}
...
Animator.prototype.Stop=function(){if(this.RestartTimerId){cancelAnimationFrame(this.RestartTimerId);this.RestartTimerId=null;}
...
Animator.prototype.Next=function(){var isEnd;if(this.Mode=='parallel'){this.FinishedCount++;isEnd=this.FinishedCount==this.NumTasks;}else{this.CurrTask++;isEnd=(this.CurrTask>=this.NumTasks);if(!isEnd){this.TaskIsActiveList[this.CurrTask]=true;this.TaskList[this.CurrTask].Start();}}
...
Animator.prototype.LoopBegin=function(){if(this.Repeat<0){return true;}else if(this.Repeat>0){return this.LoopIterator<this.Repeat;}else if(this.OnLoopBegin){return xDefBool(this.OnLoopBegin(this),true);}
return true;}
Animator.prototype.LoopEnd=function(){if(this.IsSkipToEnd())return true;if(this.Repeat!=0){this.LoopIterator++;return false;}else if(this.OnLoopEnd){return xDefBool(this.OnLoopEnd(this),false);}
return true;}
Animator.prototype.ReqAnimationFrame=function(callbackFunc){var id=this.FrameCallbackList.length;var isFirstCallback=(id==0);this.FrameCallbackList.push(callbackFunc);if(isFirstCallback&&!this.IsSkipToEnd()){var me=this;this.AnimationFrame=requestAnimationFrame(function(){me.OnFrame();});}
return id;}
...
Animator.prototype.CancelAllTimers=function(){var timerList=this.TimerList;var numTimers=timerList.length;if(numTimers==0)return;for(var i=0;i<numTimers;i++){var timer=timerList[i];if(timer)clearTimeout(timer);}
this.TimerList=[];this.TimerCallbackList=[];}
Animator.prototype.GenerateAllTimerEventsNow=function(){if(this.TimerList.length==0)return;do{var timerList=this.TimerList;var timerCallbackList=this.TimerCallbackList;this.TimerList=[];this.TimerCallbackList=[];var numTimers=timerList.length
for(var i=0;i<numTimers;i++){var timer=timerList[i];if(timer)clearTimeout(timer);}
for(var i=0;i<numTimers;i++){var callbackFunc=timerCallbackList[i];if(callbackFunc)callbackFunc();}}while(this.TimerList.length>0);}
...
Animator.prototype.UpdateModel=function(){if(this.OnModelChange&&this.NumChangedValues>0)this.OnModelChange(this.ModelRefObj,this);this.NumChangedValues=0;if(this.IsEnding){this.DoStop();}
this.IsEnding=false;}
function NewAnimation(params,parent){function LinearizeTaskDefList(taskList){var linearTaskList=[];var nTask=taskList.length;var nAnimators=0;for(var i=0;i<nTask&&nAnimators==0;i++)if(xArray(taskList[i]))nAnimators++;if(nAnimators==0)return taskList;for(var i=0;i<nTask;i++){var taskOrArray=taskList[i];if(xArray(taskOrArray)){var alen=taskOrArray.length;for(var j=0;j<alen;j++)linearTaskList.push(taskOrArray[j]);}else{linearTaskList.push(taskOrArray);}}
return LinearizeTaskDefList(linearTaskList);};parent=xDefObj(parent,null);var animator=new Animator(params,parent);var taskDefList=LinearizeTaskDefList(params.TaskList);var nTaskDefs=taskDefList.length;for(var i=0;i<nTaskDefs;i++){var task;var taskDef=taskDefList[i];if(xArray(taskDef.TaskList)){task=NewAnimation(taskDef,animator);}else{task=new AnimatorTask(taskDef,animator);}
...
ModelAnimation.prototype.ResizeStateLists=function(state){for(var i=this.NStates;i<=state;i++){this.SetStateFuncList[i]=null;this.ToStateAnimationList[i]=null;}
...
ModelAnimation.prototype.Stop=function(){if(this.PauseTimer){clearTimeout(this.PauseTimer);this.PauseTimer=null;}
...
ModelAnimation.prototype.Play=function(repeat,pauseTime){if(this.IsPlaying){this.Stop();return;}
this.Repeat=xDefNum(repeat,0);this.PauseTime=xDefNum(pauseTime,this.PauseTime);this.IsPlaying=true;this.PlayNext(0,true);}
ModelAnimation.prototype.PlayNext=function(pauseTime,restart){if(!this.IsPlaying)return;pauseTime=xDefNum(pauseTime,this.PauseTime);restart=xDefBool(restart,false);var isLastState=(this.CurrState==this.NStates-1);var replay=true;if(isLastState&&!restart){if(this.Repeat>0)this.Repeat--;replay=this.Repeat!=0;}
...
ModelAnimation.prototype.OnEndOfPause=function(){if(this.PauseTimer){clearTimeout(this.PauseTimer);this.PauseTimer=null;}
...
ModelAnimation.prototype.AnimationToState=function(id,params){if(!xAny(params.ModelRef)&&xAny(this.ModelRef)){params.ModelRef=this.ModelRef;}
